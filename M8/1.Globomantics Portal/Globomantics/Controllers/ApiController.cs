...
namespace Globomantics.Controllers
{
    public class ApiController : Controller
    {
        ...
        [HttpPost]
        [Route("api/xmla/orderHistory/customer/{customerId}")]
        public async Task<ActionResult<List<OrderHistory>>> GetOrderHistory(
            int customerId)
        {
            var xmlaApiEndpoint = "https://localhost:44317";

            return await xmlaApiEndpoint
                .AppendPathSegment("api")
                .AppendPathSegment("orderHistory")
                .AppendPathSegment("customer")
                .AppendPathSegment(customerId)
                .GetJsonAsync<List<OrderHistory>>();
        }
    }
}
